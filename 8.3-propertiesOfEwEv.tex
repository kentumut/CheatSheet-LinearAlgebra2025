...
\newline
\tsLem{8.3.5 (Transposition equality of EW)}{The eigenvalues of $A \in \mathbb{R}^{n \times n}$ are the same as those of $A^T$ But the eigenvectors can be different.
}
\newline
...
\newline
\tsIdea{Note}{If the matrix has an eigenvalue \(\lambda = 0\), then it is not invertible. Because it doesn't have a trivial nullspace.
}
