...
\newline
\tsDef{2.2 (Matrix addition, scalar multiplication)}{Addition: \(A + B = [a_{ij}+b_{ij}]{^m_{i=1}}{^n_{,j=1}}\) \(\tsPoint\) Scalar multiplication: \(\lambda A = [\lambda a_{ij}]{^m_{i=1}}{^n_{,j=1}}\)}
\newline
...
\newline
\tsDef{2.11 (Transpose)}{Mirror the matrix along its diagonal. $A = $ { $\begin{bmatrix}
                    1 & 2 & 3 \\
                    4 & 5 & 6
                \end{bmatrix}
                \leftrightarrow
...
\tsThe{2.46 (CR decomposition)}{$A = C R$.
    ...
    \textbf{Reduced REF:}
    RREF has pivots equal to 1 with zeros above and below each pivot and no zero rows
