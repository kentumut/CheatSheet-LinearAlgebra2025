...
\tsIdea{Determinant expansion along a row/column works}{
    Use:
    \[
        C_{ij}=(-1)^{i+j}\det(M_{ij})
    \]
    where \(C_{ij}\) is the cofactor, and $M_{ij}$ is obtained by deleting row $i$ and column $j$. Usually, we choose a row or a column that is zero except for one entry to simplify the expansion.
    \\ \\
    \textbf{Example:}
    \[
        \lambda I-A=
