...
\tsLem{9.2.8 (Symmetric matrix has real EW)}{A symmetric matrix $A \in \mathbb{R}^{n \times n}$ has only real eigenvalues:
    ...
    = \lambda \|v\|^2.
    \)
    Thus $\lambda = \overline{\lambda}$, so $\lambda \in \mathbb{R}$.
...
\tsDef{9.2.11 (PSD and PD matrices)}{\(A=A^\top
    ...
    \) PD matrices are invertible. (0 is not an eigenvalue) $A^{-1}$ is also PD. Because the eigenvalues are reciprocals of those of \(A\). PSD have non-negative diagonals entries. PD have positive diagonal entries. Since $x^T A x = e_i^T A e_i = A_{ii}$.
}
\newline
