...
{When  \(m < n\) then there exists a nontrivial null space, which prevents uniqueness of a solution. Let \(A \in \tsS{R}^{m \times n}\), then there exists \(\tsV{x} \neq 0\) but \(A\tsV{x} = 0\):
    \\
    \(\text{rank}(A) \leq m < n \Rightarrow dim(N(A))=n-\text{rank}(A)\)
}
\newline
...
\newline
\tsIdea{Creative Cauchy-Schwarz Inequality}{To prove
    \[
...
\tsIdea{Notes Extra}{The outer product\(A = uv^{\top}\) has always rank(A) = 1.}
\newline
Invertible matrices always have positive singular values.
\newline
If A has the eigenvalue \(\lambda \), \(A^2\) has the eigenvalue \(\lambda^2 \). 
