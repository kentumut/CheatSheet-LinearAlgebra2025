...
\tsThe{9.1.1 (Diagonalization Theorem, ability changing basis)}{$A = V \Lambda V^{-1}$, where $V$’s columns are its eigenvectors and $\Lambda$ is a diagonal matrix with $\Lambda_{ii} = \lambda_i$ and all
    other entries $0$. $A \in \mathbb{R}^{n \times n}$ and has to have a complete set of real eigenvectors (eigenbasis).
    \\ Equivalently,
    \(
    \Lambda = V^{-1} A V,
    \)
...
\newline
\tsProp{9.1.8 (Similar matrices same EV, iff complete set of EVs)}{Let $A$ and $B$ be similar matrices. Then they have the same eigenvalues. Moreover, $A$ has a complete set of eigenvectors if and only if $B$ does.
}
\newline
